    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "relative_strength_pct": round(
                float(self.relative_strength_pct),
                2,
            ),
            ...
            "chart_pattern": self.chart_pattern,

            "chart_pattern_score": round(
                float(self.chart_pattern_score),
                2,
            ),
            ...
            "confirmation_score": round(
                float(self.confirmation_score),
                2,
            ),

            "higher_timeframe_score": round(
                float(self.higher_timeframe_score),
                2,
            ),
...
def _daily_to_weekly(
    ...
    """
    Build verified weekly OHLCV candles
    from FYERS daily candles.

    No synthetic/fake price data is created.
    """
    ...
    if "timestamp" not in dataframe.columns:
        raise ValueError(
            "Timestamp required for weekly conversion."
        )
    ...
    if dataframe.empty:
        raise ValueError(
            "Unable to derive weekly candles."
        )
...
def _supertrend(
    ...
    first_position = dataframe.index.get_loc(
        first_valid
    )
...
def _detect_candlestick_pattern(
    ...
    if (
        not Config.ENABLE_CANDLESTICK_PATTERNS
        or len(dataframe) < 3
    ):
        return (
            None,
...
def _run_chart_pattern_scanner(
    ...
    *,
    ...
    if not Config.ENABLE_CHART_PATTERNS:
    ...
    try:
        ...
        # Existing pattern_scanner supports this
        # timeframe key. The candles themselves remain
        # real FYERS candles.
        pattern_timeframe = (
            "15_30_days"
        )
...
def analyze_stock(
    *,
    ...
    if not normalized_sector:
        raise ValueError(
            f"{normalized_symbol}: sector is required."
        )

    primary_df = _prepare_dataframe(
        primary_candles
    )
    ...
    if len(primary_df) < minimum_candles:
    ...
    if (
        ...
        if higher_timeframe_candles is None:
        ...
        confirmation_df = _prepare_dataframe(
            confirmation_candles
        )

        higher_df = _prepare_dataframe(
            higher_timeframe_candles
        )

        if len(confirmation_df) < 50:

            raise ValueError(
                f"{normalized_symbol}: "
                "insufficient 15-minute "
        ...
        # Swing:
        # Daily = primary
        # Weekly = derived from verified daily candles
        confirmation_df = _daily_to_weekly(
            primary_df
        )

        # For Swing the higher trend is also derived
        # from the verified Daily history.
        higher_df = confirmation_df.copy()

        if len(confirmation_df) < 30:

            raise ValueError(
                f"{normalized_symbol}: "
                "insufficient weekly candles "
                "derived from FYERS daily history."
            )

        confirmation_require_ema200 = False
        higher_require_ema200 = False

    # ========================================================
    # MULTI-TIMEFRAME
    # ========================================================

    primary_summary = _timeframe_summary(
        primary_df,
        mode=normalized_mode,
        require_ema200=True,
    )

    confirmation_summary = _timeframe_summary(
        confirmation_df,
        mode=normalized_mode,
        require_ema200=(
    ...
    higher_summary = _timeframe_summary(
        higher_df,
        mode=normalized_mode,
        require_ema200=(
    ...
    primary_bullish = bool(
        primary_summary["bullish"]
    )

    confirmation_bullish = bool(
        confirmation_summary["bullish"]
    )

    higher_bullish = bool(
        higher_summary["bullish"]
    )
    ...
    (
        ...
        dataframe["supertrend_direction"],
    ) = _supertrend(
        dataframe
    )
    ...
    if atr_value <= 0:
        ...
        raise ValueError(
            f"{normalized_symbol}: ATR unavailable."
        )
    ...
    breakout = bool(
        Config.ENABLE_BREAKOUT_ANALYSIS
        and resistance > 0
        and current_price
        >= breakout_level
    )
    ...
    if Config.ENABLE_PRICE_ACTION:
        ...
        higher_high = bool(
            len(recent_highs) >= 3
            and recent_highs.iloc[-1]
            > recent_highs.iloc[-3]
        )

        higher_low = bool(
            len(recent_lows) >= 3
            and recent_lows.iloc[-1]
            > recent_lows.iloc[-3]
        )

        if (
            Config.REQUIRE_HIGHER_HIGH
            and Config.REQUIRE_HIGHER_LOW
        ):
            price_action_bullish = bool(
                higher_high
                and higher_low
        ...
        elif Config.REQUIRE_HIGHER_HIGH:

            price_action_bullish = (
                higher_high
            )

        elif Config.REQUIRE_HIGHER_LOW:
    ...
    relative_strength_bullish = False
    ...
    if Config.ENABLE_RELATIVE_STRENGTH:
        ...
        stock_change_pct = (
            ...
            - 1.0
        ) * 100.0 if past_price > 0 else 0.0
    ...
    checks = [
        ...
        (
            ...
            "Relative strength not positive",
        ),
    ]
    ...
    if chart_pattern_confirmed:
        ...
        reasons.append(
            ...
            f"{chart_pattern or 'Bullish Pattern'}"
        )
    ...
    if confirmation_bullish:
        ...
        rejected_reasons.append(
            "Confirmation timeframe not bullish"
        )
    ...
    if (
        ...
        if ema_bullish:
            score += Config.INTRADAY_WEIGHT_TREND

        if rsi_valid:
            score += Config.INTRADAY_WEIGHT_RSI

        if macd_bullish:
            score += Config.INTRADAY_WEIGHT_MACD

        if supertrend_bullish:
            score += Config.INTRADAY_WEIGHT_SUPERTREND

        if above_vwap:
            score += Config.INTRADAY_WEIGHT_VWAP

        if volume_confirmed:
            score += Config.INTRADAY_WEIGHT_VOLUME

        if breakout:
            score += Config.INTRADAY_WEIGHT_BREAKOUT

        if price_action_bullish:
            score += Config.INTRADAY_WEIGHT_PRICE_ACTION
        ...
        if (
            ...
            score += Config.INTRADAY_WEIGHT_PATTERNS

    else:

        if ema_bullish:
            score += Config.SWING_WEIGHT_TREND

        if rsi_valid:
            score += Config.SWING_WEIGHT_RSI

        if macd_bullish:
            score += Config.SWING_WEIGHT_MACD

        if supertrend_bullish:
            score += Config.SWING_WEIGHT_SUPERTREND

        if volume_confirmed:
            score += Config.SWING_WEIGHT_VOLUME

        if breakout:
            score += Config.SWING_WEIGHT_BREAKOUT

        if price_action_bullish:
            score += Config.SWING_WEIGHT_PRICE_ACTION
        ...
        if chart_pattern_confirmed:
            score += Config.SWING_WEIGHT_CHART_PATTERN

        if candlestick_confirmed:
            score += Config.SWING_WEIGHT_CANDLE_PATTERN
    ...
    entry_buffer_percent = (
        ...
        if normalized_mode
        == Config.MODE_INTRADAY
        else Config.ENTRY_BUFFER_PERCENT_SWING
    )
    ...
    maximum_stop_percent = (
        ...
        if normalized_mode
        == Config.MODE_INTRADAY
        else Config.MAX_STOP_LOSS_PERCENT_SWING
    )
    ...
    mandatory_rules: list[bool] = []

    if Config.REQUIRE_BULLISH_EMA_STRUCTURE:

        mandatory_rules.append(
            ema_bullish
        )

    if Config.REQUIRE_SUPERTREND_BUY:

        mandatory_rules.append(
            supertrend_bullish
        )

    if Config.REQUIRE_VALID_RSI:

        mandatory_rules.append(
            rsi_valid
        )

    if Config.REQUIRE_POSITIVE_RISK_REWARD:
    ...
    if (
        ...
        if Config.INTRADAY_REQUIRE_MACD_BULLISH:

            mandatory_rules.append(
                macd_bullish
            )

        if Config.INTRADAY_REQUIRE_VOLUME_CONFIRMATION:

            mandatory_rules.append(
                volume_confirmed
            )

        if Config.INTRADAY_REQUIRE_ABOVE_VWAP:
        ...
        if Config.SWING_REQUIRE_PRICE_ABOVE_EMA20:

            mandatory_rules.append(
                current_price > ema20
            )

        if Config.SWING_REQUIRE_PRICE_ABOVE_EMA50:
    ...
    return TechnicalResult(
        ...
        primary_score=(
            primary_summary["score"]
        ),

        confirmation_score=(
            confirmation_summary["score"]
        ),

        higher_timeframe_score=(
            higher_summary["score"]
        ),
...
def analyze_for_strong_buy(
    *,
    ...
    if not result.multi_timeframe_confirmed:
        return None
