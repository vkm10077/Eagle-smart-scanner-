...
from utils.logger import build_log_extra, get_logger, log_api_call, log_exception
...
class IndexService:
    ...
    - Isolate bad symbols when FYERS rejects a batch
    ...
    def __init__(
        ...
        *,
        ...
        self.fyers_service = fyers_service or get_fyers_service()
        self.market_data_service = market_data_service or get_market_data_service()
        self.cache_service = cache_service or get_cache_service()
        self._refresh_lock = threading.RLock()

    @staticmethod
    def _normalize_full_symbol(value: Any) -> str:
        return clean_text(value).upper()

    @staticmethod
    def _extract_epoch_datetime(value: Any) -> datetime | None:
    ...
    @classmethod
    def get_supported_indices(cls) -> list[dict[str, Any]]:
        supported: list[dict[str, Any]] = []

        for index_key in cls.DEFAULT_INDEX_ORDER:
            definition = cls.INDEX_DEFINITIONS[index_key]
    ...
    @classmethod
    def normalize_index_key(cls, value: Any) -> str | None:
    ...
    @classmethod
    def _find_index_key_by_symbol(cls, symbol: Any) -> str | None:
        normalized_symbol = cls._normalize_full_symbol(symbol)

        if not normalized_symbol:
            return None

        for index_key, definition in cls.INDEX_DEFINITIONS.items():
            candidates = {
    ...
    def _normalize_index_quote(
        ...
        *,
        ...
        index_key = (
            self._find_index_key_by_symbol(full_symbol)
            or fallback_index_key
        )

        if (
            not index_key
            or index_key not in self.INDEX_DEFINITIONS
        ):
            return None

        definition = self.INDEX_DEFINITIONS[index_key]
        ...
        timestamp = (
            self._extract_epoch_datetime(timestamp_value)
            or utc_now()
        )
    ...
    @staticmethod
    def _index_cache_key(index_key: str) -> str:
    ...
    def _cache_index(
        ...
        stale_key = f"market-index-stale:{index_key}"

        self.cache_service.set(
            stale_key,
            index_data,
            ttl_seconds=self.STALE_FALLBACK_SECONDS,
        )
    ...
    def _fetch_symbol_batch(
        ...
        try:
            ...
            quote_items = self._extract_quote_items(
                response
            )
            ...
            logger.info(
                (
                    "FYERS index batch quote success | "
                ...
                extra=build_log_extra(
                    ...
                    event="index_batch_success",
                    status="success",
                    requested_symbols=len(clean_symbols),
                    received_quotes=len(quote_items),
                ),
            )
            ...
            logger.warning(
                (
                    "FYERS index batch quote failed; "
                    "switching to individual-symbol fallback | "
                    "symbols=%s | error=%s"
                ),
                clean_symbols,
                batch_exception,
        ...
        for symbol in clean_symbols:
            ...
            try:
                ...
                if not quote_items:
                    ...
                    logger.warning(
                        (
                            "FYERS returned no quote data "
                            "for index symbol | symbol=%s"
                        ),
                        symbol,
                        extra=build_log_extra(
                            component="index_service",
                            event="index_symbol_empty",
                            status="warning",
                ...
                collected_items.extend(quote_items)
                ...
                logger.info(
                    (
                        "FYERS individual index quote success | "
                        "symbol=%s"
                    ),
                    symbol,
                    extra=build_log_extra(
                        component="index_service",
                        event="index_symbol_success",
                        status="success",
                        candidate_symbol=symbol,
                    ),
                )
                ...
                logger.warning(
                    (
                        "FYERS index symbol failed and was skipped | "
                        "symbol=%s | error=%s"
                    ),
                    symbol,
                    symbol_exception,
                    extra=build_log_extra(
                        component="index_service",
                        event="index_symbol_failed",
                        status="warning",
                        candidate_symbol=symbol,
                        error=str(symbol_exception),
                    ),
                )
    ...
    def _fetch_primary_indices(
        ...
        primary_symbols = [
            self.INDEX_DEFINITIONS[key]["candidates"][0]
        ...
        try:
            quote_items = self._fetch_symbol_batch(
                access_token,
                primary_symbols,
            )

            for item in quote_items:
                normalized = self._normalize_index_quote(
                    item
                )
            ...
            log_exception(
                ...
                exception=exception,
                component="index_service",
                error_code="INDEX_PRIMARY_BATCH_FAILED",
            )
    ...
    def _fetch_index_with_candidates(
        ...
        for candidate_symbol in definition["candidates"]:
            try:
                quote_items = self._fetch_symbol_batch(
                    access_token,
                    [candidate_symbol],
    ...
    def get_index(
        ...
        *,
        ...
        normalized_key = self.normalize_index_key(
            index_key
        )
        ...
        with self._refresh_lock:
            ...
            try:
                index_data = self._fetch_index_with_candidates(
                    access_token,
                    normalized_key,
                )
                ...
                if index_data is None:
                    raise IndexDataUnavailableError(
                        (
                            "No valid live data was returned for "
                            f"{normalized_key}."
                        )
                    )
                ...
                raise IndexDataUnavailableError(
                    (
                        "Live index data is unavailable for "
                        f"{normalized_key}."
                    )
                ) from exception
    ...
    def get_dashboard_indices(
        ...
        *,
        ...
        dashboard_cache_key = self._dashboard_cache_key()

        if not force_refresh:
            cached_dashboard = self.cache_service.get(
                dashboard_cache_key
            )

            if (
                isinstance(cached_dashboard, list)
        ...
        with self._refresh_lock:
            ...
            live_results = self._fetch_primary_indices(
                access_token,
                requested_keys,
            )
            ...
            for index_key in requested_keys:
                ...
                if allow_stale_on_error:
                    stale_result = self._get_cached_index(
                        index_key,
                        allow_stale=True,
                    )
    ...
    def refresh_dashboard_indices(
        ...
        try:
            indices = self.get_dashboard_indices(
                access_token,
                force_refresh=True,
    ...
    def clear_index_cache(self) -> int:
        ...
        for index_key in self.INDEX_DEFINITIONS.keys():
            keys = (
    ...
    def health(
        ...
        if normalized_token:
            ...
            if authenticated:
                try:
                    live_indices = self.get_dashboard_indices(
                        normalized_token,
...
_global_index_service: IndexService | None = None
...
def get_index_service() -> IndexService:
    ...
    with _global_index_lock:
        if _global_index_service is None:
            _global_index_service = IndexService()
