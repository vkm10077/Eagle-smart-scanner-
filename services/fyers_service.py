...
class FyersService:
    """
    FYERS API v3 bridge for Eagle Smart Scanner.
    ...
    SWING
        Primary       = Daily
        Weekly confirmation is derived later by
        MarketDataService.
    ...
    # ========================================================
    # HISTORY API SAFE LIMITS
    # ========================================================
    #
    # We intentionally remain below FYERS hard limits.
    #
    # Minute resolution:
    #     maximum API range = 100 days
    #
    # Day / week / month:
    #     maximum API range = 366 days
    #
    # Using smaller chunks gives some safety margin.
    # ========================================================

    MINUTE_HISTORY_CHUNK_DAYS = 90

    DAILY_HISTORY_CHUNK_DAYS = 360

    SECOND_HISTORY_CHUNK_DAYS = 20
        ...
        for attempt in range(
            ...
            try:
                ...
                if payload is None:
                    ...
                    # FYERS v3 sample code uses:
                    #
                    #     fyers.history(data=data)
                    #     fyers.quotes(data=data)
                    #
                    # Use explicit data keyword first.
                    ...
                    try:
                        ...
                        # Compatibility with SDK versions
                        # accepting positional payload.
    ...
    def get_quotes(
        ...
        # FYERS quote endpoint supports a limited
        # number of symbols in each request.
        #
        # MarketDataService already batches requests.
    ...
    def _history_chunk_days(
        ...
        if category == "seconds":
            ...
            return int(
                self
                .SECOND_HISTORY_CHUNK_DAYS
            )

        return int(
            self
            .DAILY_HISTORY_CHUNK_DAYS
        )
    ...
    def get_candles(
        ...
        *,
        ...
        """
        Fetch verified candles.

        Large date ranges are automatically split
        into FYERS-safe chunks.

        Final output is timestamp-sorted and
        de-duplicated.
        """
        ...
        for (
            ...
            try:
                ...
                # Continue only if another chunk can
                # still provide useful real candles.
    ...
    def _history_date_range(
        *,
        ...
        """
        Return enough calendar history for
        technical calculations.

        All ranges stay inside normal FYERS
        limits where practical. get_candles()
        additionally chunks any larger request.
        """
        ...
        if (
            ...
            elif clean_resolution in {
                "D",
                "1D",
            }:

                # Approx 250 trading sessions.
                calendar_days = 365
        ...
        # ====================================================
            ...
            if clean_resolution == "15":
                ...
                # Stay within FYERS minute-history
                # request limit without unnecessary
        ...
        # ====================================================
            ...
            # Daily primary history.
            # ~250 exchange sessions in a normal year,
            # sufficient for EMA200 + buffer.
    ...
    def get_mode_candles(
        ...
        *,
        ...
        return {
            ...
            # MarketDataService converts these
            # verified daily candles to weekly.
